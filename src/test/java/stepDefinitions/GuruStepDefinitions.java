...
public class GuruStepDefinitions {
    ...
    GuruPage guruPage=new GuruPage(); //guru page sayfasina ulasmak icin yazdik
    @Given("cookies sorulursa kabul eder")
    public void cookies_sorulursa_kabul_eder() {
        Driver.getDriver().switchTo().frame(guruPage.cookiesIframe);// locate etcegimiz yerde iframe oldugu icin bole yazdik
    ...
    public void sutunundaki_tum_degerleri_yazdirir(String istenenSutun) {
        ...
        for (int i = 0; i < tabloBaslikListesi.size(); i++) {
            if (tabloBaslikListesi.get(i).getText().equals(istenenSutun)){
                istenenBaslikIndexi=i+1;// index 0 dan basliyor ama guru sayfasindaki liste 1 den basliyor oyuzden +1 yaptik
                break;
            }
        }
